...
namespace Rewired.Demos
{
    ...
    public class ControlRemappingDemo1 : MonoBehaviour
    {
        ...
        private void Initialize() {
            ...
            ReInput.ControllerPreDisconnectEvent +=
                JoystickPreDisconnect; // runs before joystick is completely disconnected so we can save maps
            ReInput.ControllerDisconnectedEvent +=
                JoystickDisconnected; // final disconnect that runs after joystick has been fully removed
            ResetAll();
            initialized = true;
        ...
        private void DrawInitialScreen() {
            GUIContent content;
            ActionElementMap map = ReInput.players.GetSystemPlayer().controllers.maps
                                          .GetFirstElementMapWithAction("Menu", true);
        ...
        private void DrawPage() {
            ...
            Rect screenRect = new Rect((Screen.width - (Screen.width * 0.9f)) * 0.5f,
                                       (Screen.height - (Screen.height * 0.9f)) * 0.5f, Screen.width * 0.9f,
                                       Screen.height * 0.9f);
        ...
        private void DrawPlayerSelector() {
            ...
            foreach (Player player in ReInput.players.GetPlayers(true)) {
                ...
                bool value =
                    GUILayout.Toggle(
                        prevValue, player.descriptiveName != string.Empty ? player.descriptiveName : player.name,
                        "Button", GUILayout.ExpandWidth(false));
                if (value != prevValue) {
                    // value changed
                    if (value) {
                        // selected
        ...
        private void DrawCalibrateButton() {
            ...
            Controller controller = selectedController.hasSelection
                ? selectedPlayer.controllers.GetController(selectedController.type, selectedController.id)
                : null;
            ...
            foreach (InputMapCategory category in ReInput.mapping.UserAssignableMapCategories) {
                if (!selectedPlayer.controllers.maps.ContainsMapInCategory(selectedController.type, category.id)) {
                    ...
                    if (selectedMapCategoryId < 0) {
                        ...
                        selectedMap =
                            selectedPlayer.controllers.maps.GetFirstMapInCategory(
                                selectedController.type, selectedController.id, category.id);
                    }
                }
                ...
                bool value =
                    GUILayout.Toggle(
                        prevValue, category.descriptiveName != string.Empty ? category.descriptiveName : category.name,
                        "Button", GUILayout.ExpandWidth(false));
                if (value != prevValue) {
                    // category changed
        ...
        private void DrawCategoryActions() {
            ...
            InputMapCategory
                mapCategory = ReInput.mapping.GetMapCategory(selectedMapCategoryId); // get the selected map category
            if (mapCategory == null) return;
            InputCategory
                actionCategory =
                    ReInput.mapping.GetActionCategory(mapCategory.name); // get the action category with the same name
            ...
            foreach (InputAction action in ReInput.mapping.ActionsInCategory(actionCategory.id)) {
                ...
                if (action.type == InputActionType.Button) {
                    ...
                    DrawAddActionMapButton(selectedPlayer.id, action, AxisRange.Positive, selectedController,
                                           selectedMap); // Add assignment button
                    ...
                    foreach (ActionElementMap elementMap in selectedMap.AllMaps) {
                        ...
                        DrawActionAssignmentButton(selectedPlayer.id, action, AxisRange.Positive, selectedController,
                                                   selectedMap, elementMap);
                    }
                    ...
                    if (selectedController.type != ControllerType.Keyboard) {
                        ...
                        GUILayout.BeginHorizontal();
                        GUILayout.Label(name, GUILayout.Width(labelWidth));
                        DrawAddActionMapButton(selectedPlayer.id, action, AxisRange.Full, selectedController,
                                               selectedMap); // Add assignment button
                        ...
                        foreach (ActionElementMap elementMap in selectedMap.AllMaps) {
                            ...
                            DrawActionAssignmentButton(selectedPlayer.id, action, AxisRange.Full, selectedController,
                                                       selectedMap, elementMap);
                            DrawInvertButton(selectedPlayer.id, action, Pole.Positive, selectedController, selectedMap,
                                             elementMap);
                        }
                    ...
                    // Positive action
                    string positiveName = action.positiveDescriptiveName != string.Empty
                    ...
                    // Negative action
                    string negativeName = action.negativeDescriptiveName != string.Empty
                        ? action.negativeDescriptiveName
                        : action.descriptiveName + " -";
                    GUILayout.BeginHorizontal();
                    GUILayout.Label(negativeName, GUILayout.Width(labelWidth));
                    DrawAddActionMapButton(selectedPlayer.id, action, AxisRange.Negative, selectedController,
                                           selectedMap); // Add assignment button
                    ...
                    foreach (ActionElementMap elementMap in selectedMap.AllMaps) {
                        ...
                        if (elementMap.axisContribution != Pole.Negative)
                            continue; // axis contribution is incorrect, skip
                        if (elementMap.axisType == AxisType.Normal) continue; // normal axes handled above
                        DrawActionAssignmentButton(selectedPlayer.id, action, AxisRange.Negative, selectedController,
                                                   selectedMap, elementMap);
                    }
        ...
        private void DrawActionAssignmentButton(int playerId, InputAction action, AxisRange actionRange,
                                                ControllerSelection controller, ControllerMap controllerMap,
                                                ActionElementMap elementMap) {
            if (GUILayout.Button(elementMap.elementIdentifierName, GUILayout.ExpandWidth(false),
        ...
        private void DrawInvertButton(int playerId, InputAction action, Pole actionAxisContribution,
                                      ControllerSelection controller, ControllerMap controllerMap,
                                      ActionElementMap elementMap) {
        ...
        private void DrawAddActionMapButton(int playerId, InputAction action, AxisRange actionRange,
                                            ControllerSelection controller, ControllerMap controllerMap) {
        ...
        private void DrawElementAssignmentWindow(string title, string message) {
            ...
            GUILayout.Label("Assignment will be canceled in " + ((int) Mathf.Ceil(time)).ToString() + "...",
                            style_wordWrap);
        }
        ...
        private void DrawFallbackJoystickIdentificationWindow(string title, string message) {
            ...
            bool success =
                ReInput.controllers.SetUnityJoystickIdFromAnyButtonOrAxisPress(entry.joystickId, 0.8f, false);
            if (!success) return;
        ...
        private void DrawCalibrationWindow(string title, string message) {
            ...
            if (entry.selectedElementIdentifierId >= 0) {
                ...
                // Records Min/Max
                bool newRecording =
                    GUILayout.Toggle(entry.recording, "Record Min/Max", "Button", GUILayout.ExpandWidth(false));
        ...
        private bool ProcessJoystickAssignmentChange(JoystickAssignmentChange entry) {
            ...
            dialog.StartModal(entry.id, DialogHelper.DialogType.JoystickConflict, new WindowProperties {
                                  ...
                                  message =
                                      "This joystick is already assigned to another player. Do you want to reassign this joystick to " +
                                      player.descriptiveName + "?",
        ...
        private bool ProcessAddOrReplaceElementAssignment(ElementAssignmentChange entry) {
            ...
            if (entry.state == QueueEntry.State.Confirmed) {
                ...
                if (conflictFoundEventData != null) {
                    ...
                    newEntry.changeType =
                        ElementAssignmentChangeType.ConflictCheck; // set the new type to check for conflicts
                    actionQueue.Enqueue(newEntry); // enqueue the new entry
                }
 ...
 Application.platform == RuntimePlatform.OSXEditor || Application.platform == RuntimePlatform.OSXPlayer || Application.platform == RuntimePlatform.OSXWebPlayer;
                ...
                if (isOSX) {
                    message =
                        "Press any key to assign it to this action. You may also use the modifier keys Command, Control, Alt, and Shift. If you wish to assign a modifier key itself to this action, press and hold the key for 1 second.";
                }
                else {
                    message =
                        "Press any key to assign it to this action. You may also use the modifier keys Control, Alt, and Shift. If you wish to assign a modifier key itself to this action, press and hold the key for 1 second.";
                }
        ...
        private bool ProcessElementAssignmentConflictCheck(ElementAssignmentChange entry) {
            ...
            if (conflictFoundEventData.isProtected) {
                string message = conflictFoundEventData.assignment.elementDisplayName +
                                 " is already in use and is protected from reassignment. You cannot remove the protected assignment, but you can still assign the action to this element. If you do so, the element will trigger multiple actions when activated.";
                ...
                string message = conflictFoundEventData.assignment.elementDisplayName +
                                 " is already in use. You may replace the other conflicting assignments, add this assignment anyway which will leave multiple actions assigned to this element, or cancel this assignment.";
        ...
        private void SetGUIStateStart() {
            ...
            pageGUIState =
                guiState && !busy && !dialog.enabled &&
                !dialog.busy; // enable page gui only if not busy and not in dialog mode
        ...
        private void JoystickPreDisconnect(ControllerStatusChangedEventArgs args) {
            ...
            if (selectedController.hasSelection && args.controllerType == selectedController.type &&
                args.controllerId == selectedController.id) {
        ...
        public void IdentifyAllJoysticks() {
            ...
            foreach (Joystick joystick in ReInput.controllers.Joysticks) {
                actionQueue.Enqueue(
                    new FallbackJoystickIdentification(joystick.id,
                                                       joystick.name)); // enqueue each joystick for identification
            }
        }
        ...
        protected void CheckRecompile() {
            ...
            if (UnityEditor.EditorApplication.isCompiling) {
                ...
                GUILayout.Window(0, GetScreenCenteredRect(300, 100), RecompileWindow,
                                 new GUIContent("Scripts are Compiling"));
                return;
            }
        ...
        private class DialogHelper
        {
            private const float
                openBusyDelay =
                    0.25f; // a small delay after opening the window that prevents assignment input for a short time after window opens

            private const float
                closeBusyDelay =
                    0.1f; // a small after closing the window that the GUI will still be busy to prevent button clickthrough
            ...
            private float busyTimer {
                get {
                    if (!_busyTimerRunning) return 0.0f;
            ...
            public void StartModal(int queueActionId, DialogType type, WindowProperties windowProperties,
                                   Action<int, UserResponse> resultCallback) {
                StartModal(queueActionId, type, windowProperties, resultCallback, -1.0f);
            }

            public void StartModal(int queueActionId, DialogType type, WindowProperties windowProperties,
                                   Action<int, UserResponse> resultCallback, float openBusyDelay) {
            ...
            public void Draw() {
                ...
                bool origGuiEnabled = GUI.enabled;
                GUI.enabled = true;
                GUILayout.Window(windowProperties.windowId, windowProperties.rect, drawWindowFunction,
